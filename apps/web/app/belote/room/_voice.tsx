 ...
 * @description Appel AUDIO + VIDÉO WebRTC (mesh) pour la room Belote (Phase 3).
 *   La caméra + le micro s'activent automatiquement à l'entrée dans la room.
 *   Signaling via le gateway /webrtc ; relai média par le TURN/STUN SALISTAR
 *   (ICE servers récupérés sur /api/turn-creds). Le serveur ne voit jamais le
 *   flux : pur P2P, relayé par TURN si le P2P direct échoue.
 */
'use client';
...
/** Tuile vidéo : attache un MediaStream à un <video>. */
...
export default function VoiceCall({ roomCode, token }: { roomCode: string; token: string | null }) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      startedRef.current = false;
      setError(e?.name === 'NotAllowedError' ? 'Caméra/micro refusés — autorise l’accès dans le navigateur' : (e?.name === 'NotFoundError' ? 'Aucune caméra/micro détecté' : (e?.message || 'Échec de l’appel')));
    }
  }, [token, roomCode, makePeer, closePeer]);

  // Démarre automatiquement caméra + micro à l'entrée dans la room
  useEffect(() => {
    if (token && roomCode) start();
    return () => { hangUp(); };
  ...
  const toggleMic = useCallback(() => {
    ...
    if (!t) return; t.enabled = !t.enabled; setMicOn(t.enabled);
  }, []);
  const toggleCam = useCallback(() => {
    const t = localStreamRef.current?.getVideoTracks()[0];
    if (!t) return; t.enabled = !t.enabled; setCamOn(t.enabled);
  }, []);
  ...
  return (
    <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: 12 }}>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {!inCall && !error && (
          <span style={{ color: BLUE, fontSize: '0.82rem' }}>Activation de la caméra…</span>
        )}
        {error && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: '#FCA5A5', fontSize: '0.8rem' }}>{error}</span>
            <button onClick={start} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: `linear-gradient(90deg, ${GOLD}, #F59E0B)`, color: '#0A1535', fontWeight: 800, border: 'none', borderRadius: 999, padding: '6px 14px', cursor: 'pointer' }}>
              <Phone style={{ width: 15, height: 15 }} /> Réessayer
            </button>
          </div>
        )}
      </div>
      ...
      {inCall && (
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <span style={{ color: BLUE, fontSize: '0.76rem' }}>
            {remotes.length > 0 ? `${remotes.length} autre(s) en ligne` : 'En attente d’un autre joueur…'}
          </span>
        </div>
      )}
    </div>
...
const ctrl = (active: boolean): React.CSSProperties => ({
  display: 'inline-flex', alignItems: 'center', gap: 6,
  background: active ? 'rgba(239,68,68,0.2)' : 'rgba(255,255,255,0.1)',
  color: active ? '#FCA5A5' : '#fff',
  border: '1px solid rgba(255,255,255,0.15)', borderRadius: 999, padding: '8px 14px', cursor: 'pointer', fontWeight: 700, fontSize: '0.82rem',
});
